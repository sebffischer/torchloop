...
LLTM_API void* _lltm_run_script_module (void* network, void* loss_fn, void* input, void* target, void* optimizer) {
  try {
    return  make_raw::Tensor(lltm_run_script_module(from_raw::ScriptModule(network), from_raw::GraphFunction(loss_fn), from_raw::Tensor(input), from_raw::Tensor(target), from_raw::SGD(optimizer)));
  } LLTM_HANDLE_EXCEPTION
  return (void*) NULL;
}
